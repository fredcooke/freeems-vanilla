...
//	CoreVars->DRPM = Counters.; currently in use logging *ticksPerDegree as at wed nzst 3:10pm 16 march 2011 see line ~98 of derivedVarsGenerator.c and line ~277 of coreVarsGenerator.c
	...
	// Currently spare:
	CoreVars->DDRPM = injectorMainPulseWidthsMath[3];
	CoreVars->DTPS = postReferenceEventDelays[3];
	CoreVars->DMAP = outputEventInputEventNumbers[3];
	...
	KeyUserDebugs.zsp9 = injectorMainPulseWidthsMath[2];
	KeyUserDebugs.zsp10 = postReferenceEventDelays[2];
	KeyUserDebugs.zsp11 = outputEventInputEventNumbers[2];
}
